...
const ForgotPassword = () => {
  ...
  const { seller, isLoading } = useSelector(state => state.sellerReducer,shallowEqual);
  ...
  const onEmailSend = (e) => {
    ...
    if (emailRegex.test(email)) {
      dispatch(actionCreators.sendSellerEmail(email));
    } else {
  ...
  useEffect(() => {
    if (otp) {
      router.push(`/seller/passwordreset/${otp._id}`);
    }
  }, [otp, router, dispatch]);
