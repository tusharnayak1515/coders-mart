...
const CartButtonDiv = dynamic(
  () => import("../../../components/CartButtonDiv"),
  { ssr: false }
);
  ...
  return (
    <div className={styles.productPage}>
      <Head>
        ...
        <meta
          name="keywords"
          content={`nextjs, e-commerce, coders-mart, ${product?.name}`}
        />
      </Head>
      ...
      <div className={styles.product_topDiv}>
        ...
        <div className={styles.product_details_div}>
          ...
          {product?.size && (
            <h3 className={styles.product_size}>
              Size: <span>{product?.size}</span>
            </h3>
          )}
          <h2 className={styles.product_price}>₹{product?.price}</h2>
          {product?.quantity !== 0 ? (
            <h3 className={styles.product_quantity}>
              Hurry! Only {product?.quantity} left!
            </h3>
          ) : (
            <h3 className={styles.product_quantity} style={{ color: "red" }}>
              Out of stock!
            </h3>
          )}
    ...
    if (store.getState().productReducer.product) {
      ...
      if (cookieObj.cm_user_token) {
        await store.dispatch(
          actionCreators.userProfile(cookieObj.cm_user_token)
        );
        ...
        await store.dispatch(
          actionCreators.sellerProfile(cookieObj.cm_seller_token)
        );
      }
    } else {
      return {
